...
namespace itis {
	...
	std::optional<std::string> HashTable::Search(int key) const {
		...
		for (const auto &lst : buckets_.at(hash(key))) {
			if (lst.first == key) {
				return lst.second;
			}
		}
	...
	void HashTable::Put(int key, const std::string &value) {
		// Tip 1: compute hash code (hashIndex) to determine which bucket to use
		// Tip 2: consider the case when the key exists (read the docs in the header file)

		if (buckets_.at(hash(key)).empty()) {
			num_keys_++;
			auto pair = std::pair<int, std::string>();
			pair.first = key;
			pair.second = value;
			buckets_.at(hash(key)).push_back(pair);
	...
	std::optional<std::string> HashTable::Remove(int key) {
		...
		for (auto &pair : buckets_.at(hash(key))) {
			if (pair.first == key) {
				auto a = pair.second;
				buckets_.at(hash(key)).remove(pair);
				return pair.second;
			}
		}
		return std::nullopt;
